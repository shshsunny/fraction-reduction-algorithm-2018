...
class MainWindow(QMainWindow):
    ...
    def onSelectCase(self):  # 选择测试用例
        # 获取选择的文件
        fn = QFileDialog.getOpenFileName(self, 'Select a testcase', 'Testcases/', 'Text Files (*.txt)')[0]
        if not fn: return
        self.filename = fn
    ...
    def onLoadResult(self):  # 从结果库导入数据并分析和绘制
        # 获取导入结果库文件的名称
        filename = QFileDialog.getOpenFileName(self, 'Select a result file', 'Results/', 'CSV Files (*.csv)')[0]
