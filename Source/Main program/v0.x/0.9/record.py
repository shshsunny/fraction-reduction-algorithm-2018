...
def test(testlist:list, case:list) -> list:  # 对每个算法进行测试之后，输出多列算法测试结果
    ...
    for algo in testlist:  # 依次测试每个算法
        ...
        # 如果是RD算法，提示是否需要的是空间监视
        if algo == "RD":
            if input("Do you want to run RDMM instead of RD? [y/n]") == 'y':
                data.append(RDMMtest(case))
                continue
        elif algo == "Subtract":
            if input("Do you want to run SubtractCTM instead of Subtract? [y/n]") == "y":
                data.append(SubtractCTMtest(case))
            continue
